...
def fetchCommitsBranchesReleasesData(repo_name,repo_key):
    ...
    data = requests.get(usersUrl, headers)
    data = data.json()
    print(data)
    url = data['repos_url']
    page_no = 1
...
def getcommitscsv(repos_df,repo_name):
    for i in range(len(repos_df)):
        response = requests.get(repos_df.loc[i, 'Commits URL'], headers)
        response.json()
    commits_information = []
    ...
    dataFrame.to_csv(BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_COMMITS_DATA' + '.csv', index = False,header=True)
    path_of_commits_csv = BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_COMMITS_DATA' + '.csv'
    return path_of_commits_csv

def getbranchescsv(repos_df,repo_name):
    for i in range(len(repos_df)):
        response = requests.get(repos_df.loc[i, 'Branches URL'], headers)
    ...
    for i in range(repos_df.shape[0]):
        ...
        while (True):
            ...
            for branch in response:
                ...
                print(branch_commit_url)
                response_branch = requests.get(branch_commit_url, headers)
    ...
    dataFrame_branches.to_csv(BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_BRANCHES_DATA' + '.csv', index = False,header=True)
    path_of_branches_csv = BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_BRANCHES_DATA' + '.csv'
...
def getreleasescv(repos_df,repo_name):
    ...
    dataFrameForReleases.to_csv(BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_RELEASE_DATA' + '.csv', index = False,header=True)
    path_of_releases_csv = BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_RELEASE_DATA' + '.csv'
    return path_of_releases_csv

def getcontributerscsv(repos_df,repo_name):
    for i in range(len(repos_df)):
        response = requests.get(repos_df.loc[i, 'Contributors URL'],headers)
        response.json()
    ...
    dataFrame_contributers.to_csv(BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_CONTRIBUTERS_DATA' + '.csv', index = False,header=True)
    path_of_contributers_csv = BASE_IMAGE_PATH + LOCAL_REPO_PATH + repo_name + '_CONTRIBUTERS_DATA' + '.csv'
...
def predict_for_created_issues(file,LOCAL_IMAGE_PATH,CREATED_ISSUES_IMAGE_NAME):
    ...
    axs.legend()
    axs.set_title('Created Issues Forecast')
    axs.set_xlabel('Date')
    axs.set_ylabel('Created Issues')
    # Save the figure in /static/imag
...
def predict_for_closed_issues(file,LOCAL_IMAGE_PATH, CLOSED_ISSUES_IMAGE_NAME):
    ...
    axs.xaxis.set_major_formatter(mdates.AutoDateFormatter(locator))
    axs.legend()
    axs.set_title('Clossed Issues Forecast')
    axs.set_xlabel('Date')
    axs.set_ylabel('Closed Issues')
    # Save the figure in /static/imag
...
def predict_for_pulls(filePath,LOCAL_IMAGE_PATH, PULLS_FORECAST_IMAGE_NAME):
    ...
    df_list_pull = DataFrame(list_of_pulls)
    ...
    model.fit(pullsDataFrame)
    future_Pulls = model.make_future_dataframe(periods=365)
    forecast = model.predict(future_Pulls)
...
def predict_for_commits(file,LOCAL_IMAGE_PATH, COMMITS_FORECAST_IMAGE_NAME):
    ...
    model.plot(forecast, ax=axs)
    axs.legend()
    axs.set_title('Commits Forecast')
    axs.set_xlabel('Date')
...
def predict_for_branches(file,LOCAL_IMAGE_PATH, BRANCHES_FORECAST_IMAGE_NAME):
    ...
    model.plot(forecast, ax=axs)
    axs.legend()
    axs.set_title('Branches Forecast')
    axs.set_xlabel('Date')
...
def predict_for_releases(file,LOCAL_IMAGE_PATH, RELEASES_FORECAST_IMAGE_NAME):
    ...
    model.plot(forecast, ax=axs)
    axs.legend()
    axs.set_title('Releases Forecast')
    axs.set_xlabel('Date')
...
def predict_for_contributers(file,LOCAL_IMAGE_PATH, CONTRIBUTERS_FORECAST_IMAGE_NAME):
    ...
    model.plot(forecast, ax=axs)
    axs.legend()
    axs.set_title('Contributers Forecast')
    axs.set_xlabel('Date')
    axs.set_ylabel('Releases')
